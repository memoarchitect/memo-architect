...
//
// Mapping:
//   - Kinds → owl:Class with rdfs:subClassOf for layer grouping
//   - Relationships → owl:ObjectProperty with domain/range
//   - Closure rules → owl:Restriction (cardinality where possible)
//   - CoSMA layers → owl:Class (grouping superclasses)
//   - Viewpoints → owl:AnnotationProperty groupings
// ─────────────────────────────────────────────────────────────────────────────
...
export function exportToOwlTurtle(
    ...
    namespace: string = 'https://sysand.dev/ontology/memo#'
): string {
    const lines: string[] = [];
    const prefix = 'memo';
    ...
    if (config.cosmaLayers && config.cosmaLayers.length > 0) {
        lines.push('# ─── CoSMA Layers (grouping superclasses) ────────────────────────');
        for (const layer of config.cosmaLayers) {
            ...
            lines.push(`    rdfs:comment "CoSMA visualization layer: ${escape(layer.label)}" .`);
            lines.push('');
        }
    }

    // ─── Kind classes ──────────────────────────────────────────────────
    lines.push('# ─── Entity Kinds ────────────────────────────────────────────────');
    for (const [kindName, kindDef] of Object.entries(config.kinds)) {
        ...
        // Add closure rule restrictions for this kind
    ...
    lines.push('# ─── Relationship Types (Object Properties) ──────────────────────');
    ...
    // ─── Viewpoints as named individuals ───────────────────────────────
    if (config.viewpoints && config.viewpoints.length > 0) {
        lines.push('# ─── Viewpoints ──────────────────────────────────────────────────');
        lines.push(`${prefix}:Viewpoint a owl:Class ;`);
        lines.push(`    rdfs:label "Viewpoint" .`);
        lines.push('');
        for (const vp of config.viewpoints) {
            lines.push(`${prefix}:${vp.id} a ${prefix}:Viewpoint ;`);
            lines.push(`    rdfs:label "${escape(vp.label)}" .`);
            lines.push('');
        }
    }
