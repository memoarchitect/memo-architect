...
export function DiagramEditor({ diagramId }: DiagramEditorProps) {
    ...
    // Sync diagram → text on diagramId change
    useEffect(() => {
        if (diagram && model) {
    ...
    // Re-sync text when switching into a text-bearing mode
    useEffect(() => {
        if ((mode === 'text' || mode === 'split') && diagram && model) {
            setTextContent(serializeDiagramToSysML(diagram, model.elements));
        }
    // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [mode]);

    // Clear parsing indicator when a parse result arrives
    useEffect(() => {
        setIsParsing(false);
    }, [parseErrors]);
