...
import {
    compactRoute, routeDirectOrthogonal, routeDirectOrthogonalEdges, routeOrthogonalEdges,
    sameSegment, segmentsCrossOrthogonally,
...
//
// Both routers, the geometry they share, and the crossing-penalised lattice the
// tidy pass searches now live in `orthogonal-router.ts`. They are re-exported
// here because this module is the layout facade every view imports from, and
// moving a router should not move every call site.

export type { RoutePoint, RouteObstacle, OrthogonalRouteRequest, TidyRouteOptions } from './orthogonal-router';
// Re-exported from the local bindings above, not with `export … from`. A name
// that is both imported for use here and re-exported through a second statement
// resolved to undefined at runtime under Vite's HMR interop, which showed up as
// "routeOrthogonalEdges is not defined" inside the IBD template.
export { routeDirectOrthogonal, routeDirectOrthogonalEdges, routeOrthogonalEdges, compactRoute };
