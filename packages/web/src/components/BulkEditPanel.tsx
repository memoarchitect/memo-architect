...
function ValueHint({ values }: { values: string[] }) {
    ...
    if (unique.length === 0) return <span style={{ color: COLOR.faint, fontSize: '11px' }}>—</span>;
    if (unique.length === 1) return <span style={{ color: COLOR.muted, fontSize: '11px' }}>{unique[0]}</span>;
    return (
        <span style={{ color: '#F59E0B', fontSize: '11px' }}>{unique.length} different values</span>
    );
}
        ...
        >
            ...
            >
                <div>
                    ...
                    <div style={{ color: COLOR.muted, fontSize: '11px', marginTop: '2px' }}>
                ...
                <button
                    ...
                    style={{ color: COLOR.muted, background: '#F0F0ED', fontSize: '11px' }}
            ...
            <div className="px-4 py-3 flex-shrink-0" style={sectionStyle()}>
                <div style={{ color: COLOR.muted, fontSize: '11px', fontWeight: 600, marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                    Selection
                </div>
                <div className="flex flex-wrap gap-1">
                    {kinds.map(k => (
                        <span
                            ...
                            style={{ background: '#EEF2FF', color: '#4F46E5', fontSize: '11px', fontWeight: 500 }}
                        >{k}</span>
                    ))}
                </div>
                <div style={{ color: COLOR.faint, fontSize: '11px', marginTop: '6px' }}>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3" style={sectionStyle()}>
                <div style={{ color: COLOR.muted, fontSize: '11px', fontWeight: 600, marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                    Common Attributes
                        ...
                        {commonKeys.map(key => (
                            <div key={key}>
                                <div className="flex items-center justify-between mb-1">
                                    <span style={{ color: COLOR.secondary, fontSize: '11px', fontWeight: 500 }}>{key}</span>
