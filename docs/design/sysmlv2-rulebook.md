...
```
**MEMO rule:** MEMO `DiagramView : View` and `DocumentBackedView : View` are view definitions. Every concrete view in `memo::profile::views::*` is a `view <id> : DiagramView { … }` with one view per file.
...
### 18.4 View — `memo::profile::views::risk::matrix`
...
These seven packages form the current domain coverage. The rule book defines
how to author their SysML consistently; the ontology owns their domain content.
